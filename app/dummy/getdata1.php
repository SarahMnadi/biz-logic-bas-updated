...
//ANGALIA KAMA THE REQUEST SENT HAS THE FINGERPRINTID AND THE DEVICE_ID(device token)
if (isset($_GET['FingerID']) && isset($_GET['device_token'])) {
    ...
    //FIND THE DEVICE IN THE DATABASE BY MATCHING THE DEVICE_ID
    else{
        ...
        if ($row = mysqli_fetch_assoc($resultl)){
            ...
            //KAMA DEVICE_MODE NI 1 (attendance) LOGIN OR LOGOUT THE USER
            if ($device_mode == 1) {
            ...
            //KAMA DEVICE_MODE NI 0 Enroll the staff by adding his fingerprint_id in database
...
//PULL THE FINGER_ID OF THE NEW STAFF FOR MAPPING WITH THE FINGER PRINT TEMPLATE IN THE DATABASE
...
//CHECK IF THE DEVICE MODE IS ATTENDANCE OR ENROLLMENT
...
//CONFIRM (by setting status/add-fingerid = 0) THAT THE FINGER_ID HAS BEEN SUCCESFUL BINDED WITH FINGERPRINT TEMPLATE
...
//CHECK IF USER NEEDS TO BE DELETED AND RETURN THE FINGER ID
